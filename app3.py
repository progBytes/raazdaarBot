...
"""
This is the extended version of app2.py, where the app2.py had a shared password management and was not working as we intended but in this file:
each user will start their own bot, set their own password in dm with bot itself, then the bot will be used in DM with a person (added in that conversation) and will encrypt the message of the person who has set up a password in the bot.
for example:
I start the bot in dm with the bot, set a password, then i message Seyar for instance, then add this bot in that chat, so now that i am authenticated to the bot, all my messages will go as 'hey there' to Seyar and he can only see the messages by knowing the password of mine, otherwise always will see 'hey there', now if Seyar wants the same thing, he will set his own bot and that way his messages will be protected too.
"""
...
def main():
    ...
    logger.info("Bot starting... Delete bot_data.json for fresh setup! Setup in private DM first.")
